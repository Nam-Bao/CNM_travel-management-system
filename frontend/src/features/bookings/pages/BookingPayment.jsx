...
const BookingPayment = () => {
  ...
  // Nhận dữ liệu từ trang chọn ghế truyền sang
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      // 1. Gửi Đơn hàng (Kèm theo mảng ghế đã chọn)
      const payload = {
        tourId: tour._id,
        guest_size: guestSize,
        contact_info: contactInfo,
        selected_beds: selectedBeds || [], // 🔥 Truyền mảng ghế xuống Backend
      ...
      if (paymentMethod === "VNPAY") {
        ...
        if (vnpayRes.data && vnpayRes.data.url) {
          window.location.href = vnpayRes.data.url; // Chuyển hướng sang VNPay
        } else {
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-2 font-medium">
                  💺 Vị trí đã chọn: <span className="text-blue-600 font-bold">{selectedBeds?.length > 0 ? selectedBeds.join(", ") : "Hãng bay tự động sắp xếp"}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-xl border-t-4 border-orange-500 sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="text-xs text-gray-500">* Nếu chọn đặt cọc 50% thì phần còn lại phải thanh toán trước 7 ngày</div>
            </div>
            ...
            <div className="space-y-3 mb-6">
              <label className={`flex items-center p-3 border rounded-xl cursor-pointer transition ${paymentMethod === 'VNPAY' ? 'border-blue-500 bg-blue-50/50' : 'hover:bg-gray-50'}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Hỗ trợ thẻ ATM, Visa, MasterCard</p>
                </div>
              </label>
              ...
              {/* <label className={`flex items-center p-3 border rounded-xl cursor-pointer transition ${paymentMethod === 'CASH' ? 'border-blue-500 bg-blue-50/50' : 'hover:bg-gray-50'}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Thanh toán tại văn phòng trước 7 ngày.</p>
                </div>
              </label> */}
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-xs text-gray-600 leading-relaxed">
                  Tôi đã đọc và đồng ý với <a href="#" className="text-blue-600 hover:underline">Điều khoản đặt tour</a> và <a href="#" className="text-blue-600 hover:underline">Chính sách hoàn hủy</a>.
                </span>
              </label>
            </div>
