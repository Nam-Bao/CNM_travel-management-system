...
const AddTourPage = () => {
    ...
    // YÊU CẦU 1: Thêm State cho URL Ảnh và Lỗi Form
    ...
    const removeItineraryDay = (indexToRemove) => {
        if (itinerary.length === 1) return alert('Tour phải có ít nhất 1 ngày lịch trình!');
    ...
    const validateLogic = () => {
        ...
        if (imageFiles.length === 0 && imageUrls.trim() === '') {
            setFormError('❌ LỖI LOGIC: Vui lòng chọn ảnh từ máy tính hoặc nhập Link URL ảnh!'); return false;
        }
        return true;
    };
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-10">
            <div className="max-w-5xl mx-auto bg-white p-8 rounded-xl shadow-lg mt-6 border-t-4 border-blue-600">
                <h2 className="text-3xl font-black text-gray-800 mb-6 border-b pb-4">🌟 Thêm Tour Du Lịch Mới</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="bg-blue-50 p-6 rounded-lg border border-blue-100">
                        <h3 className="text-xl font-bold text-blue-800 mb-4">📌 Thông tin chung</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-gray-700 mb-1">Tên Tour *</label>
                                <input type="text" name="title" value={formData.title} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Thời lượng *</label>
                                <input type="text" name="duration" value={formData.duration} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">% Khuyến mãi</label>
                                <input type="number" min="0" max="100" name="sale_percentage" value={formData.sale_percentage} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Ngày đi *</label>
                                <input type="date" name="start_date" value={formData.start_date} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Ngày về *</label>
                                <input type="date" name="end_date" value={formData.end_date} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1 text-red-600">Tổng số vé *</label>
                                <input type="number" name="max_seats" value={formData.max_seats} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border border-red-300 rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1 text-red-600">Ghế trống hiện tại *</label>
                                <input type="number" name="available_seats" value={formData.available_seats} onChange={handleBasicChange} required disabled={loading} className="w-full px-4 py-2 border border-red-300 rounded focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-gray-700 mb-1">Mô tả tổng quan *</label>
                                <textarea name="description" value={formData.description} onChange={handleBasicChange} required rows="3" disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-green-50 p-6 rounded-lg border border-green-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Người lớn *</label>
                                <input type="number" name="adult" value={price.adult} onChange={handlePriceChange} required disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-green-500 font-bold text-green-700" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Trẻ em (2-12 tuổi)</label>
                                <input type="number" name="child" value={price.child} onChange={handlePriceChange} disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-green-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Em bé (&lt; 2 tuổi)</label>
                                <input type="number" name="infant" value={price.infant} onChange={handlePriceChange} disabled={loading} className="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-green-500" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-orange-50 p-6 rounded-lg border border-orange-100">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-orange-800">🗺️ Lịch trình chi tiết</h3>
                            <button type="button" onClick={addItineraryDay} disabled={loading} className="bg-orange-500 text-white px-3 py-1 rounded text-sm font-bold">+ Thêm ngày</button>
                        </div>
                        <div className="space-y-4">
                            {itinerary.map((item, index) => (
                                <div key={index} className="bg-white p-4 border rounded-lg relative">
                                    ...
                                    <div className="pr-16 space-y-2">
                                        <input type="text" placeholder="Tiêu đề ngày..." value={item.title} onChange={(e) => handleItineraryChange(index, 'title', e.target.value)} required disabled={loading} className="w-full px-3 py-1.5 border rounded text-sm" />
                                        <textarea placeholder="Mô tả..." value={item.description} onChange={(e) => handleItineraryChange(index, 'description', e.target.value)} required rows="2" disabled={loading} className="w-full px-3 py-1.5 border rounded text-sm"></textarea>
                                    </div>
                    ...
                    <div className="bg-purple-50 p-6 rounded-lg border border-purple-100">
                        <h3 className="text-xl font-bold text-purple-800 mb-4">📸 Hình ảnh Tour (Tối đa 5 ảnh)</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Cách 1: Tải từ máy tính</label>
                                <input type="file" multiple accept="image/*" onChange={handleFileChange} disabled={loading || imageUrls.length > 0} className="w-full bg-white border p-2 rounded text-sm disabled:opacity-50" />
                            </div>
                            <div className="text-center font-black text-gray-400">HOẶC</div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Cách 2: Nhập Link URL (Ngăn cách bằng dấu phẩy)</label>
                                <textarea rows="2" placeholder="https://anh1.jpg, https://anh2.jpg" value={imageUrls} onChange={(e) => setImageUrls(e.target.value)} disabled={loading || imageFiles.length > 0} className="w-full px-4 py-2 bg-white border rounded text-sm focus:ring-2 focus:ring-purple-500 disabled:opacity-50" />
                            </div>
                        </div>
                    </div>

                    <button type="submit" disabled={loading} className="w-full py-4 rounded-xl text-lg font-black text-white bg-blue-600 hover:bg-blue-700 shadow-lg">
