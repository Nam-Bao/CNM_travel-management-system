  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const actualCustomers = allUsers.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg transform hover:scale-105 transition duration-300">
          ...
          <div className="text-3xl font-black">
            {stats.totalTours} <span className="text-lg font-medium">tour</span>
          </div>
        ...
        {recentTours.length > 0 ? (
          <div className="space-y-4">
            {recentTours.map((tour, index) => {
              // KIỂM TRA NGÀY KHỞI HÀNH CỦA TỪNG TOUR
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl ${isTourEnded ? "bg-gray-100 text-gray-400" : "bg-blue-100 text-blue-600"}`}
                    >
                      {index + 1}
                    </div>
                    <div>
                      <h4
                        className={`font-bold ${isTourEnded ? "text-gray-400 line-through" : "text-gray-800"}`}
                      >
                        {tour.title}
                      </h4>
                  ...
                  <div className="text-right">
                    ...
                    <div
                      className={`font-bold ${isTourEnded ? "text-gray-400" : "text-red-500"}`}
                    >
                      {formatTourPrice(tour.price)}
                    </div>

                    {/* ĐOẠN LOGIC HIỂN THỊ TRẠNG THÁI CHỖ NGỒI */}
                    {isTourEnded ? (
                      <div className="text-xs font-medium text-gray-500 px-2 py-1 bg-gray-100 rounded-full inline-block mt-1">
                        Đã kết thúc
                      </div>
                    ) : (
