    ...
    <header className="bg-white shadow-sm sticky top-0 z-50 border-b h-20 flex items-center">
      <div className="max-w-7xl mx-auto px-4 w-full flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-8 font-bold text-gray-600">
            ...
            <Link to="/tours" className="hover:text-blue-600 transition">Khám phá Tour</Link>
            <Link to="/about" className="hover:text-blue-600 transition">Về chúng tôi</Link>
          ...
          {currentUser ? (
            <div className="flex items-center gap-4 bg-gray-50 p-1.5 pr-4 rounded-full border border-gray-100 shadow-sm">
              ...
              <div className="hidden md:flex items-center mx-2 pl-4 border-l border-gray-200 gap-4">
                <Link to="/my-bookings" className="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase tracking-wider flex items-center gap-1">
                  <span>✈️</span> Lịch sử
                </Link>
                {currentUser.role === 'admin' && (
                  <Link to="/admin" className="text-gray-600 hover:text-blue-600 font-bold text-xs uppercase tracking-wider">
                    ⚙️ Quản trị
                  </Link>
                )}
              </div>
            ...
            <Link to="/auth" className="bg-blue-600 text-white px-5 py-2.5 rounded-full font-bold hover:shadow-lg transition text-sm">
              Đăng nhập
            </Link>
          )}
        </div>
